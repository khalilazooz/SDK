...
/***************************************************************/
  sint_16	lCD_Init(){
	#if        LCD_MODE    ==  8
gpio_set_port_direction(GPIO_PORT_D,GPIO_ALL_OUTPUT);
gpio_set_pin_direction(GPIO_PORT_C,RS,GPIO_OUTPUT);
gpio_set_pin_direction(GPIO_PORT_C,EN,GPIO_OUTPUT);
gpio_set_pin_direction(GPIO_PORT_C,RW ,GPIO_OUTPUT);
...
sint_16 lcd_char(uint_8 char_data)  /* LCD data write function */
{
	...
	_delay_us(2);
	 gpio_set_pin_value(LCD_Command_Port,EN ,LOW);
	_delay_us(2);/* Data write delay */
return lcd_success;
}

sint_16 lcd_string(uint_8 *str)		/* Send string to LCD function */
{
	for(uint_8 i=0;str[i]!= '\0';i++)		/* Send each char of string till the NULL */
	{
		lcd_char(str[i]);
	}
	return lcd_success;
